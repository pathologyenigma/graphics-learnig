...
mod two_perlin_spheres {
    ...
    pub fn gen() -> HittableList {
        ...
        objects.add(Rc::new(RefCell::new(Sphere::new(
            Point3::new((0., -1000., 0.)),
            1000.,
            Rc::new(RefCell::new(Lambertian::with_texture(Rc::new(RefCell::new(pertext))))),
        ))));
        objects.add(Rc::new(RefCell::new(Sphere::new(
            Point3::new((0., 2., 0.)),
            2.,
...
mod earth {
    use std::{cell::RefCell, rc::Rc};
    use ray_tracing_the_next_week::{HittableList, ImageTexture, Lambertian, Point3, Sphere, TEXTURE_PATH};

    pub fn gen() -> HittableList {
        
        let earth_texture = ImageTexture::new((String::from(TEXTURE_PATH) + "earthmap.jpg").as_str());
        let earth_surface = Lambertian::with_texture(Rc::new(RefCell::new(earth_texture)));
        let globe = Rc::new(RefCell::new(Sphere::new(Point3::new((0., 0., 0.)), 2., Rc::new(RefCell::new(earth_surface)))));
...
fn main() {
    ...
    eprintln!("witch type you want to render, 1 means a random scene, 2 means two_spheres, 3 means two_perlin_spheres, 4 means a earth sphere");
